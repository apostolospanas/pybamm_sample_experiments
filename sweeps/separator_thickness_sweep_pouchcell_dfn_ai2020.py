...
# Sweep the separator thickness (µm)
separator_thickness_um_list = [10, 16, 20, 25, 30]  # edit as you wish

# Timestamp & output dir
stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_DIR = Path(__file__).resolve().parent / f"results_sep_sweep_{stamp}"
...
# Show default separator thickness
default_sep_thickness_m = float(base_param["Separator thickness [m]"])
print(f"Default Ai2020 separator thickness: {default_sep_thickness_m*1e6:.1f} µm")
...
for sep_um in separator_thickness_um_list:
    sep_m = sep_um * 1e-6
    tag = f"{MODEL_NAME}_sep{sep_um:.0f}um"
    ...
    if os.path.exists(csv_file):
        ...
        # Copy base params and only change the separator thickness
        p = base_param.copy()
        p.update({"Separator thickness [m]": sep_m})

        # Build a fresh simulation (reuse same model object is fine)
...
plt.title(f"Voltage vs Time — Separator thickness sweep ({PARAM_SET})")
...
plt.title(f"Discharge Energy vs Cycle — Separator sweep ({PARAM_SET})")
plt.legend(); plt.grid(True); plt.tight_layout(); plt.show()

# ---------------------------
# SAVE SUMMARY
# ---------------------------
summary_df = pd.DataFrame(summaries).sort_values("Separator [um]")
summary_file = out("separator_sweep_summary.csv")
